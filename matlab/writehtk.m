function writehtk(file,d,fp,tc,bTrans,type)
...
ct = fwrite(fid,nf,'int32');
ct = fwrite(fid,round(fp*1.E7),'int32');
ct = fwrite(fid,nv*4,'int16');
ct = fwrite(fid,tc,'int16');

if bTrans
    ct = fwrite(fid,d,'float32');
else
    ct = fwrite(fid,d','float32');
end
