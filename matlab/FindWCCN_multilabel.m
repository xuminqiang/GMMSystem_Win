...
function [eigs_vecs,eigs_mags] = FindWCCN_multilabel(supervec,labels,K_largest,output)
...
models.spk_mean = zeros( dim, nclass );
for i = 1 : nclass
    ind = find( labels(i,:) == 1 );
    if ~isempty(ind),
        models.spk_mean(:,i) = mean(supervec0(:,ind),2);
        if bPair ~= 1,
