function [ DATA, HTKCode ] = htkread( Filename, mode )
...
nSamp = fread(fid,1,'int32',mode);

% Read sampPeriod
sampPeriod = fread(fid,1,'int32',mode);

% Read sampSize
sampSize = fread(fid,1,'int16',mode);

% Read HTK Code
HTKCode = fread(fid,1,'int16',mode);
...
if bitget(HTKCode, 11),
    ...
    A = fread(fid,[1 DIM],'float',mode);
B = fread(fid,[1 DIM],'float',mode);
    
    % Read and uncompress the data
    DATA = fread(fid, [DIM nSamp], 'int16',mode)';
    ...
    DATA = fread(fid, [DIM nSamp], 'float',mode)';
end
